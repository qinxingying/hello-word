...
public:
    ...
    int CreateViews(QWidget* parent_ , int eType_);//创建视图
    void SetCurrentGroupId(int nGroupId_);//设置当前组
    static QSize GetMinimumWidgetSize(int eType_);//获取最小的窗口
    static int IsSizeAvailable(int nId_, QWidget* parent_);//

    int CreateViews2(QWidget*);//创建视图2
    //创建各种视图显示
    ...
    void ShowDefectInfo(int nGroupId_, int id);//显示缺陷信息
