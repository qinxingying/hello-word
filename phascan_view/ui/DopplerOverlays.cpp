	...
	QObject(parent)
{
	if(parent)
	{
        ...
        m_pView->GetDataViewConfigure(&m_nGroup , &m_nLaw , &m_eDisp) ;//获取组数据信息：组别、光束、显示模式（A显示、B显示......）
	}
	else
	{
		m_pView   = 0 ;
...
{
    switch(m_eDisp)//根据不同的显示模式显示焊缝、厚度、闸门
	{
...
{
    GROUP_CONFIG&		   _group = m_pConfigure->group[m_nGroup];//具体某一组的信息
    DopplerViewItems* _pItemGroup = m_pView->GetItemGroup() ;//某一组的项目图元
    if(_group.eTxRxMode == setup_TX_RX_MODE_TOFD)//eTxRxMode-收发模式
	{
		if(_group.bShowLwBw) {
...
//	if(_group.bShowGate)
//	{    //闸门
        _pItemGroup->EnableItems(OVERLAYS_GATE);//使能闸门图元
        _pItemGroup->SetGateDrawMode(GATE_MODE_GATE_HORIZENTAL);//设置闸门数据绘画模式-水平闸门
        int _nScanPos = m_pProcess->GetScanIndexPos();//获取超声坐标轴位置的索引（第几个刻度）  m_pProcess：在构造函数中实例
		PEAK_CONFIG _info[setup_GATE_MAX];
        m_pProcess->GetGatePeakInfos(m_nGroup, _nScanPos, m_nLaw, _info);//获取峰值信息
    ...
    //}
        ...
        //光标
    if(_group.bShowCursor)//显示光标
	{
        int _ScanIndex = m_pProcess->GetScanIndexPos();//获取超声坐标轴位置的索引（第几个刻度）

        //获取参考光标的高度（绿色）
		_group.afCursor[setup_CURSOR_A_REF] = m_pProcess->GetPeakTraceHeight(m_nGroup,
										   _ScanIndex,
										   m_nLaw,
										   _group.afCursor[setup_CURSOR_U_REF]);/* 参考光标在超声轴的位置 */
        //获取测量光标的高度（红色）
    ...
    //更新视图
    _pItemGroup->UpdateItems();//更新项（重点）
...
{
    DopplerViewItems* _pItemGroup  = m_pView->GetItemGroup();//获取组图元项 m_pView（DopplerDataView）
    _pItemGroup->EnableItems(OVERLAYS_NONE);//未使能
    CreateOverlays();//创建图元
    m_pView->UpdateScene();//刷新场景
}
