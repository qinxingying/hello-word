...
//    }
    ...
    ui->ComGeometry->setCurrentIndex(m_pGroup->part.eGeometry);//设置索引
	UpdateGeometryState();
...
void DopplerGroupTab::UpdateGeometryState()
{
    ...
    }else if(_eGeometry == setup_PART_GEOMETRY_OD||_eGeometry==setup_PART_GEOMETRY_ID){
        ui->LabelPartSize1->setText(QString(tr("Thickness:")));
        ui->LabelPartSize2->setText(QString(tr("Length:")));
...
//        ui->ValuePartSize3->hide();
        ...
        ui->ValuePartSize3->setValue(m_pGroup->part.afSize[2]);
        ui->ValuePartSize4->setValue(m_pGroup->part.afSize[3]);
    }else if(_eGeometry == setup_PART_GEOMETRY_ASY)
    {

     ui->LabelPartSize1->setText(QString(tr("厚度1:")));
     ui->ValuePartSize1->setValue(m_pGroup->part.afSize[0]);
     ui->LabelPartSize2->setText(QString(tr("厚度2:")));
     ui->ValuePartSize2->setValue(m_pGroup->part.afSize[4]);
     ui->ValuePartSize2->setDisabled(false);
...
{
    //m_pGroup 组参数的对象
    m_pGroup->bShowThickness = checked;//显示厚度的标志位

    ProcessDisplay _display ;//  窗口显示实例对象
    _display.UpdateAllViewOverlay();//更新所有窗口的光标、闸门
}
...
{
    //DopplerConfigure
    DopplerConfigure* Configure = DopplerConfigure::Instance();//打开文件读取数据配置信息对象（读取的原始数据）
    ParameterProcess* _process = ParameterProcess::Instance();//读取的数据参数存储配置对象（后期设置的数据）
    GROUP_CONFIG& group = Configure->group[m_nGroupId];
    if(group.part.weldFormat) //判断焊缝是2代还是1代
    {
        DialogWeldFixDataII dataIIDialog(this, m_nGroupId);//实例2代焊缝对话框
        if(dataIIDialog.exec())//按下OK后把焊缝数据保存到工件结构体中，当按下显示复选框的时候才显示
        {
            //ParameterProcess* _process = ParameterProcess::Instance();
    ...
    {
        DialogDxfFileLoad dialog(this);//实例1代焊缝对话框
    ...
    group.part.weld_border = _process->GetWeldBorder(m_nGroupId);//获取焊缝边界

    ProcessDisplay _display;//数据显示实例对象
    _display.UpdateAllViewOfGroup(m_nGroupId);//Description: 更新所有指定组窗口，包括标尺、标题、光标
    _display.UpdateAllView();// Description: 更新窗口的全部设置,Input: 【pWidget_：窗口指针】
    _display.UpdateAllViewOfGroup(m_nGroupId);//更新所有指定组窗口，包括标尺、标题、光标
    g_pMainWnd->RunDrawThreadOnce(true);//Description: 全部窗口(DopplerDataView)扫查数据更新一次
    ui->ComSkewAngle->setCurrentIndex(m_pGroup->eSkew);//探头角度
    ui->ValuePartSize1->setValue(m_pGroup->part.afSize[0]);//工件厚度
...
{
    ParameterProcess* _process = ParameterProcess::Instance();
    _process->SetupShowWeldPart(m_nGroupId , checked); //checked=true则显示
