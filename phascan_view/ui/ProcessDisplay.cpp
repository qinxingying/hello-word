...
void ProcessDisplay::UpdateDataViewFrameAH(DopplerDataView* pWidget_, int nGroupId_, int nLawId_)
{
    ...
    if(_group.eTravelMode == setup_TRAVEL_MODE_TRUE_DEPTH && _group.eGroupMode != setup_GROUP_MODE_PA){
        ...
        pWidget_->SetTofdSampleStart(sampleStart, DopplerDataView::DATA_VIEW_RULER_BOTTOM);
        _process->SetTofdSampleStart(nGroupId_, sampleStart);
...
void ProcessDisplay::UpdateDataViewFrameAV(DopplerDataView* pWidget_ , int nGroupId_ , int nLawId_)
{
	...
	//---------------
    if(_group.eTravelMode == setup_TRAVEL_MODE_TRUE_DEPTH && _group.eGroupMode != setup_GROUP_MODE_PA){
        float sampleStart = _group.afCursor[setup_CURSOR_TOFD_CAL] - _group.fCalibratedSoundDepth + _fStart;
        pWidget_->SetTofdSampleStart(sampleStart, DopplerDataView::DATA_VIEW_RULER_LEFT);
        _process->SetTofdSampleStart(nGroupId_, sampleStart);
