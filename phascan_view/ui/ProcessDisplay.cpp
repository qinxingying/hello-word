...
void ProcessDisplay::UpdateDataViewFrame(DopplerDataView* pWidget_)
{
	...
	setup_DISPLAY_MODE _eMode  ;
	pWidget_->GetDataViewConfigure( &_nGroupId ,  &_nLawId ,  &_nDisplay);
    _eMode = (setup_DISPLAY_MODE)_nDisplay  ;//显示模式
	switch(_eMode)
	{
	case  setup_DISPLAY_MODE_A_H:
...
{
	DopplerOverlays _overlay(pWidget_);
    _overlay.UpdateOverlays();//焊缝、厚度等显示
    UpdateDataViewTitle(pWidget_); //更新指定窗口的标题 pWidget_ 窗口标题，传入数据显示指针
}
...
{
    for(int j = 0 ; j < g_pMainWnd->GetDisplayTableQty() ; j++) //标签页数量(有多少页)
	{
        QList<QWidget*>* pList = g_pMainWnd->GetDisplayTableWidgetList(j);//每页有多少个视图列表-窗口
		for(int i = 0 ; i < pList->count() ; i++)
		{
            UpdateDataViewOverlay((DopplerDataView*)pList->at(i));// 更新指定窗口的光标、闸门
		}
	}
}
...
void ProcessDisplay::UpdateAllViewFrameOfGroup(int nGroupId_)
{
	...
	for(int j = 0 ; j < g_pMainWnd->GetDisplayTableQty() ; j++)
	{
		...
		for(int i = 0 ; i < pList->count() ; i++)
		{
            ...
            if(_nGroupId == nGroupId_)//如果是同一组的数据
                UpdateDataViewFrame(_pView);//更新指定窗口的标尺
		}
	}
}
...
/****************************************************************************
  Description: 更新所有指定组窗口，包题、光括标尺、标标
  Input:   【nGroupId_ ： 组ID】
*****************************************************************************/
