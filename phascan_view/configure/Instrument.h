...
{
    setup_WELD_SYMMETRY = 0 , //对称
    setup_WELD_RIGHT ,//右边跛口
    setup_WELD_LEFT//左边跛口
} ;
...
struct WELD
{
    ...
    float	weland_height;  //焊缝高度（钝边高度-除去跛口那一部分）
    float	weland_offset;  //焊缝偏移量（相对于中心线的偏移）
    float	fizone_height;
	float	fizone_angle ;
	float	fizone_radius;
	float	weldtype_pos ;
...
struct WELD_II
{
    ...
    double eTopThinkness;                //TKY腹板厚度 --也可以加不等厚工件厚度在此以及其它消边信息
...
struct PART_CONFIG
{
	...
	//geometry parameter
    float				afSize[5];				// sizes if necceray , part height , width , diameter
	MATERIAL			material;				/* 材料 */
    WELD_FORMAT         weldFormat;
	WELD				weld;					// weld
    WELD_II             weld_ii;
    float               weld_border; //焊缝边界
    bool                CADFresh;
...
enum setup_CURSOR_TYPE
{
	setup_CURSOR_LAW = 0 , /* 参考光标当前聚焦法则*/
        setup_CURSOR_A_REF   , /* 参考光标的高度  reference*/
        setup_CURSOR_A_MES   , /* 测量光标的高度  measurement*/
