...
//          qDebug() <<"****************************\n";
    ...
    //解析组的数量（一定先是组的配置参数解析出来，其它连带的参数才能解析）
...
{
    //m_shape 在上一层函数中已经层层解析下来，在组的数据中已经有了m_shape ，所以可以比较了--数字相比
...
void Config::getWeldData( int groupId, WELD_II & weld_ii)
{
    ...
    Paramters::Weld &weld = m_groups[groupId].m_specimen.m_weld;//板焊缝
         ...
         weld_ii.ASY.H1=weld.m_Main_V.m_height;
          weld_ii.ASY.W2=weld.m_Slave_V.m_width;
           weld_ii.ASY.H2=weld.m_Slave_V.m_height;
            ...
            qDebug()<<"*************hcx**********";
            qDebug()<<"W1"<<weld_ii.ASY.W1<<"H1"<<weld_ii.ASY.H1<<"W2"<<weld_ii.ASY.W2<<"H2"<<weld_ii.ASY.H2<<"\n";
            qDebug()<<"M_thickness"<<weld_ii.ASY.M_thickness<<"S_thickness"<<weld_ii.ASY.S_thickness<<"\n";
             qDebug()<<"*************hcx**********";
        break;
    default:
        break;
...
//    }
    ...
    /* Specimen */
    targetGroup.part.Geometry     = currentSpecimen.m_shape;//把形状值赋给组信息的Geometry(数字)
    ...
    if(Paramters::Specimen::PLANE == currentSpecimen.m_shape) {
        targetGroup.part.Thickness[0]    = currentSpecimen.m_geometry.m_plane.m_height * 1000.0;
        targetGroup.part.Diameter     = 0;
    } else if(Paramters::Specimen::CYLINDER == currentSpecimen.m_shape) {
        targetGroup.part.Thickness[0]    = (currentSpecimen.m_geometry.m_cylinder.m_outside -
        ...
        targetGroup.part.Thickness[0]    = (currentSpecimen.m_geometry.m_nozzle.m_outside -
        ...
        if(Paramters::Fillet::WED_1 == currentSpecimen.m_geometry.m_fillet.m_probePos
            ...
            targetGroup.part.Thickness[0] = currentSpecimen.m_geometry.m_fillet.m_topPlane.m_height * 1000.0;
        }else{
            targetGroup.part.Thickness[0] = currentSpecimen.m_geometry.m_fillet.m_bottomPlane.m_height * 1000.0;
        }
    }else if(Paramters::Specimen::ASYMMETRIC_PLANE == currentSpecimen.m_shape){
        //不等厚焊缝厚度
        targetGroup.part.Thickness[0]=currentSpecimen.m_geometry.m_Asymmetric_Plane.m_MainPlane.m_height*1000.0;
        targetGroup.part.Thickness[1]=currentSpecimen.m_geometry.m_Asymmetric_Plane.m_SlavePlane.m_height*1000.0;
    }
