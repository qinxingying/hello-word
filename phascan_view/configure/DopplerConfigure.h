/***************************
  ...
  Date : 2013-08-26
  Description:   Áî®‰∫éÊìç‰ΩúÈÖçÁΩÆÊñá‰ª∂ÔºåË¥üË¥£ÈÖçÁΩÆÊñá‰ª∂ÁöÑÂä†ËΩΩ
				 ÂàùÂßãÂåñÔºåÂíå‰øùÂ≠ò
************************************/
...
struct FOCALLAW_RESULT
{
	...
	float   afBeamExitPointX[setup_MAX_GROUP_LAW_QTY];  /*ÂÖ•Â∞ÑÁÇπÁöÑÂùêÊ†á*/
	float   afBeamExitPointY[setup_MAX_GROUP_LAW_QTY];
	float   afBeamExitPointZ[setup_MAX_GROUP_LAW_QTY];

	//ÂèëÂ∞ÑÊé¢Â§¥
	float afFocusPosX[setup_MAX_GROUP_LAW_QTY];/*ËÅöÁÑ¶ÁÇπÂùêÊ†á*/
	float afFocusPosY[setup_MAX_GROUP_LAW_QTY];/*ËÅöÁÑ¶ÁÇπÂùêÊ†á*/
	float afFocusPosZ[setup_MAX_GROUP_LAW_QTY];/*ËÅöÁÑ¶ÁÇπÂùêÊ†á*/
	float afElementPosX[setup_MAX_GROUP_LAW_QTY][setup_MAX_PROBE_ELEMENT];/*ÈòµÂÖÉÂùêÊ†á*/
	float afElementPosY[setup_MAX_GROUP_LAW_QTY][setup_MAX_PROBE_ELEMENT];/*ÈòµÂÖÉÂùêÊ†á*/
	float afElementPosZ[setup_MAX_GROUP_LAW_QTY][setup_MAX_PROBE_ELEMENT];/*ÈòµÂÖÉÂùêÊ†á*/
	float afRefractPosX[setup_MAX_GROUP_LAW_QTY][setup_MAX_PROBE_ELEMENT];/*ÂÖ•Â∞ÑÁÇπÂùêÊ†á */
	float afRefractPosY[setup_MAX_GROUP_LAW_QTY][setup_MAX_PROBE_ELEMENT];/*ÂÖ•Â∞ÑÁÇπÂùêÊ†á */
	float afRefractPosZ[setup_MAX_GROUP_LAW_QTY][setup_MAX_PROBE_ELEMENT];/*ÂÖ•Â∞ÑÁÇπÂùêÊ†á */
};
...
struct coupleCScanGroupInfo{
    bool calculateState;      //±æ◊ÈC…®◊¥Ã¨£¨falseŒ™ªπŒ¥º∆À„£¨trueŒ™“—æ≠º∆À„
    float oldPartThickness;   //æ…µƒπ§º˛∫Ò∂»÷µ£¨”√”⁄≈–∂œπ§º˛∫Ò∂»∏ƒ±‰÷ÿ–¬º∆À„ÒÓ∫œ’¢√≈ƒ⁄µƒ÷µ
    QVector<WDATA> data;      //¥¢¥ÊÒÓ∫œC…® ˝æ›
};
