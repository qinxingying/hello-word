...
class Doppler3DAllPrivate
{

public:
    Doppler3DAllPrivate(Doppler3DAll *threeDAll)
        : q(threeDAll), xRot(180.0f), yRot(0.0f), zRot(0.0f),
          zTra(-20.0f), m_nGroupId(-1), m_pGroup(NULL), m_pScanner(NULL),
          m_pDraw(NULL), m_pAngleZoom(NULL), m_pDrawRate(NULL), m_pDataNo(NULL),
          m_nWidth(10), m_nHeight(10)
    {
        memset(&m_ptOrigin, 0x00, sizeof(GLPOINT));
        //timer = new QTimer(this);
        //connect(timer, SIGNAL(timeout()), q, SLOT(advanceGL()));
...
void Doppler3DAllPrivate::BeginDraw()
{
...
void Doppler3DAllPrivate::InitGLPoint(GLPOINT* point_, int iLen_)
{
...
void Doppler3DAllPrivate::drawFrame(bool bPositive_, GLdouble y0_, GLdouble x1_, GLdouble x2_)
{
    GLdouble _sign = 1;
    if(!bPositive_)
        _sign = -1;
...
void Doppler3DAllPrivate::drawWeld(bool bPositive_, GLdouble y0_, GLdouble* x1_, GLdouble* x2_, int bMirroring_)
{
...
void Doppler3DAllPrivate::drawIShapeWeld(bool bPositive_, GLdouble y0_, GLdouble* x1_, GLdouble* x2_, int bMirroring_)
{
...
void Doppler3DAllPrivate::drawVShapeWeld(bool bPositive_, GLdouble y0_, GLdouble* x1_, GLdouble* x2_, int bMirroring_)
{
    GLdouble _sign = 1;
    if(!bPositive_)
        _sign = -1;
...
void Doppler3DAllPrivate::drawDoubleVShapeWeld(bool bPositive_, GLdouble y0_, GLdouble* x1_, GLdouble* x2_, int bMirroring_)
{
    Q_UNUSED(bMirroring_);
    GLdouble _sign = 1;
    if(!bPositive_)
...
void Doppler3DAllPrivate::drawDefect(GLdouble y0_)
{
    GROUP_CONFIG& _group = m_pConfig->group[m_nGroupId] ;
    if(setup_GROUP_MODE_PA == _group.eGroupMode)
    {
...
void Doppler3DAllPrivate::drawDefectAzimuthal(GLdouble y0_)
{
    GROUP_CONFIG& _group = m_pConfig->group[m_nGroupId] ;
...
void Doppler3DAllPrivate::drawDefectLiner(GLdouble y0_)
{
    GROUP_CONFIG& _group = m_pConfig->group[m_nGroupId] ;
    LAW_CONFIG* _law = m_process->GetLawConfig(m_nGroupId);
...
void Doppler3DAllPrivate::CreateMatrixBuff()
{
    ReleaseMatrixBuff();
...
void Doppler3DAllPrivate::ReleaseMatrixBuff()
{
...
void Doppler3DAllPrivate::CalcMatrixAzimuthal()
{
    GROUP_CONFIG& _group = m_pConfig->group[m_nGroupId] ;
    LAW_CONFIG*     _law = m_process->GetLawConfig(m_nGroupId);
    int _iAngle = (int)m_process->GetProbeAngle(m_nGroupId);
...
void Doppler3DAllPrivate::CalcMatrixLinear()
{
    GROUP_CONFIG& _group = m_pConfig->group[m_nGroupId] ;
    LAW_CONFIG*     _law = m_process->GetLawConfig(m_nGroupId);
    int _iAngle = (int)m_process->GetProbeAngle(m_nGroupId);
...
int Doppler3DAllPrivate::DrawPixbuff(GLdouble y0_)
{
    U8* _pMarker = m_process->GetScanMarker(m_nGroupId)  ;
...
void Doppler3DAllPrivate::SetGroupId(int nGroupId_)
{
...
void Doppler3DAllPrivate::drawUShapeWeld(bool bPositive_, GLdouble y0_, GLdouble* x1_, GLdouble* x2_, int bMirroring_)
{
...
Doppler3DAll::Doppler3DAll(int nGroupId_, QWidget *parent)
    : QGLWidget(parent), d(new Doppler3DAllPrivate(this))
{
...
void Doppler3DAll::paintGL()
{
    ...
    glTranslatef(0.0f, 0.0f, d->zTra);

    glRotatef(d->xRot, 1.0f, 0.0f, 0.0f);
    glRotatef(d->yRot, 0.0f, 1.0f, 0.0f);
    glRotatef(d->zRot, 0.0f, 0.0f, 1.0f);
    ...
    {
        _bMirroring = i % 2;
        switch(d->m_pWeld->eSymmetry)
        {
        case setup_WELD_SYMMETRY:
            d->drawWeld(true, _y0, &_x1, &_x2, _bMirroring);  d->drawFrame(true, _y0, _x1, _x2);
            d->drawWeld(false, _y0, &_x1, &_x2, _bMirroring); d->drawFrame(false, _y0, _x1, _x2);
            break;
...
void Doppler3DAll::keyPressEvent(QKeyEvent *event)
{
    ...
    {
        memset(&d->m_ptOrigin, 0x00, sizeof(GLPOINT));
        d->xRot = 180.0f;
        d->yRot = 0.0f;
        d->zRot = 0.0f;
        d->zTra = -20.0f;
...
{
    int dx = event->x() - d->lastPos.x();
    int dy = event->y() - d->lastPos.y();
...
{
    event->delta() > 0 ? d->m_ptOrigin.z += 1.0f : d->m_ptOrigin.z -= 1.0f;
    updateGL();
}
