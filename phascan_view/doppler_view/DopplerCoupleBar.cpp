...
void DopplerCoupleSScanBar::paintEvent(QPaintEvent *)
{
    ...
    int _nColor;
    float  _fScale = pow(10.0,(m_pConfig->group[nGroupId].CoupleGain + CUR_RES.Couple_Com_Gain[nGroupId])/20.0);
...
void DopplerCoupleCScanBar::paintEvent(QPaintEvent *)
{
    ...
    float step;
    int _nColor;
    float  _fScale = pow(10.0,(_group.CoupleGain + CUR_RES.Couple_Com_Gain[nGroupId])/20.0);
    if( start < scanStart){
        //全白
        if(end <= scanStart){
            if(m_dirction){
