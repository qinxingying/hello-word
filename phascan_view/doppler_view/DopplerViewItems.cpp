...
{
    UpdateItemsDefect();//更新缺陷
	UpdateScanMarker();
	UpdateItemsLwBw();
	UpdateItemsGate();
    UpdateItemsCursor() ;//更新光标项
    UpdateItemsLawMarker() ;
    UpdateItemsLawMarkerCScan();

    UpdateItemsThickness() ;//更新厚度项
    UpdateItemsWeldBorder();
    UpdateItemsTOPCWidth();
    UpdateItemsWeld();//更新焊缝
}
...
{
	int i ;
    if(!(m_eShow & OVERLAYS_THICKNESS)) // ?&0x02==0 如果不是厚度匹配
	{
		for(i = 0 ; i < 10 ; i++)
            if(m_pThickness[i]) m_pThickness[i]->hide();//所有厚度视图项隐藏
		return ;
	}

    double _fStart , _fStop , _fSliderStart , _fSliderStop;//什么起点？什么终点？
    bool tofdDepth = false;
    float pcs;
    if(m_bHorizental){//扫查轴平行-bool（PA-相控阵模式，自发自收）
        //获取标尺的范围-底部
    ...
    if(tofdDepth){
        ...
        if(_fStart > _fStop)//起点大于终点时候，交换
        {
            double _fTmp = _fStop ;
            _fStop = _fStart ;
            _fStart= _fTmp   ;
        }
        //m_fInterval-工件厚度值,随着数据框中的数值变化而变化
        int _nQty =(int)(_fStop / m_fInterval) - (int)(_fStart / m_fInterval)  ;//一共显示多少条厚度线
        if(_nQty > 10) _nQty = 10 ;
        float _fStartPos = (1 + (int)(_fStart / m_fInterval)) * m_fInterval ;//第一条厚度的位置

        QRectF _rect(0 , 0 , 0 , 0);//使用浮点精度定义平面中的矩形
        for(i = 0 ; i < _nQty ; i++)//显示_nQty条厚度线
        {
            if(!m_pThickness[i])//如果厚度图元数组不为NULL
            {
                m_pThickness[i]  = new DopplerLineItem(COLOR_THICKNESS);//COLOR_THICKNESS-灰色
                m_pThickness[i]->SetLineStyle(Qt::DashLine) ;//设置线风格 DashLine（由几个像素分隔的破折号）
                m_pThickness[i]->SetItemType(DOPPLER_GRAPHICS_ITEM_THICKNESS);//设置图元类型
                m_pThickness[i]->SetItemId(i);//图元ID
                m_pDataView->AddOverlayItems(m_pThickness[i]);//添加覆盖项-将厚度图元添加到场景中
            }
            //设置厚度线的类型(水平还是垂直)
            m_pThickness[i]->SetLineType(m_bHorizental ? DopplerLineItem::LINE_VERTICAL : DopplerLineItem::LINE_HORIZENTAL);
            //线是否能移动 LINE_MOVE_NO
            ...
            //水平视图？设置矩形左边缘
             m_bHorizental ? _rect.setLeft(_fStartPos + i * m_fInterval) : _rect.setTop(_fStartPos + i * m_fInterval)  ;
             //m_bHorizental ? _rect.setLeft(20 + i * m_fInterval) : _rect.setTop(20 + i * m_fInterval)  ;
            //设置图元在场景的几何形状
        ...
        for(i = _nQty ; i < 10 ; i++)
        {
            if(m_pThickness[i])  m_pThickness[i]->hide(); //如果为空，则全部隐藏-不可见
        }
    }
}
...
void DopplerViewItems::UpdateItemsWeld()
{
	...
	{
        QPainterPath _path; //绘图容器-焊缝路径
        QPainterPath _HAZpath;//热影响区路径
        bool brushHAZ;//刷新热影响区
        DrawWeld(_path, _HAZpath, brushHAZ);//绘制焊缝
		if(m_pLawMarker){
...
//绘制不等厚焊缝顶缝对齐 收工.
void DopplerViewItems::DrawWeldASYDataII(QPainterPath &path)//不等厚焊缝
{
    ...
    m_pDataView->GetRulerRange(&_fStartV2 , &_fStopV2 , &_fSliderStartV2, &_fSliderStopV2, DopplerDataView::DATA_VIEW_RULER_BOTTOM ) ;
            ...
            if(setup_PLANE_TOP==m_cPart.weld_ii.Align)
              {
                  ...
                  // 厚板3个点
                  _pos[0].setX(-W1-W3/2);
                  _pos[0].setY(h1-h1/l1*W1); // 利用角度相同计算纵坐标
                  _pos[1].setX(-W3/2);
            ...
            }else if(setup_PLANE_CENTER==m_cPart.weld_ii.Align)
            {
                ...
                _pos[5].setX(W3/2);
                _pos[5].setY(T1/2-T2/2+T2);//薄板的厚度
            ...
            }else
            {
                ...
                _pos[1].setY( h1-h1/l1*W1+H1);
                _pos[2].setX(-W3/2);
                _pos[2].setY(T1-h2);//厚板的厚度（削下边后的点）
...
//绘制I型焊缝收工，要素:位置坐标、中心线、焊缝的钝边高度、宽度（偏移）-焊缝的形状
void  DopplerViewItems::DrawWeldI (QPainterPath& path)
{
    QPointF _pos[2] ; //浮点精度的点 （两点确定一条直线）
	double _fStartV , _fStopV , _fSliderStartV, _fSliderStopV;
    //获取左坐标范围
	m_pDataView->GetRulerRange(&_fStartV , &_fStopV , &_fSliderStartV, &_fSliderStopV, DopplerDataView::DATA_VIEW_RULER_LEFT ) ;
	...
	_pos[0] = m_pDataView->TranslateToScenePlan(&_pos[0]) ;
	_pos[1] = m_pDataView->TranslateToScenePlan(&_pos[1]) ;
    path.moveTo(_pos[0]);//移动点位置（作为绘图路径起点）
    path.lineTo(_pos[1]);//从当前位置到pos[1]点绘制直线线

    //绘制左右两边的焊缝以及倒置焊缝（将点坐标关于Y轴对称即可）
    int _nStart = _fStartV / m_fInterval ;//m_fInterval 工件厚度
	int _nStop  = _fStopV  / m_fInterval + 1;
	for(int i = _nStart ; i < _nStop ; i++)
	{
        if(i%2)//一个正一个倒，这个是倒的 I为1 3 5的时候成立
		{
            double _nOffsetY = m_fInterval * ( i + 1 ) ;//工件厚度的间隔
            ...
            double _nOffsetY = m_fInterval * i ;//工件厚度的间隔（i可为0，从0开始）
			if(m_cPart.weld.eSymmetry == setup_WELD_SYMMETRY || m_cPart.weld.eSymmetry == setup_WELD_LEFT )
			{
                _pos[0].setX(-m_cPart.weld.weland_offset);//X轴的偏移-
                _pos[0].setY(_nOffsetY); //Y轴的偏移-相对于工件厚度
				_pos[1].setX(-m_cPart.weld.weland_offset);
                _pos[1].setY(m_cPart.weld.weland_height + _nOffsetY);//加上第一个的焊缝高度

                _pos[0] = m_pDataView->TranslateToScenePlan(&_pos[0]) ;//转换场景坐标
				_pos[1] = m_pDataView->TranslateToScenePlan(&_pos[1]) ;
                path.moveTo(_pos[0]);
                path.lineTo(_pos[1]);
