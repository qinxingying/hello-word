...
/****************************************************************************
  Description: 换算ITEM（图元）位置
*****************************************************************************/
void DopplerDataView::SetItemGeometry(DopplerGraphicsItem* item , QRectF& rect_)
{
    //QPointF 点
    QPointF _pos1 = rect_.topLeft() ;//左下角位置
    QPointF _pos2 = rect_.bottomRight() ;//右下角位置
    _pos1 = TranslateToScenePlan(&_pos1)  ;//转换为场景坐标（像素为单位）
	_pos2 = TranslateToScenePlan(&_pos2)  ;
    QRectF _rect ( _pos1 ,  _pos2) ;//左上角右下角确定矩形形状
    item->SetItemGeometry(_rect);//设置图元形状-矩形
    item->SetItemGeometryReal(rect_);//设置图元形状-矩形
}
