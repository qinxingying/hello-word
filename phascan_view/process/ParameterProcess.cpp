...
int  ParameterProcess::SetupPart(int nGroupId_ , PART_CONFIG* part_)
{
    ...
    memcpy((void*)&_part, (void*)part_, sizeof(PART_CONFIG))  ;//将后期配置的信息覆盖原始数据
	return 0 ;
}
...
{
	SCANNER& _scaner = m_pConfig->common.scanner ;
    //将超声轴坐标值转换为数据索引index  fDist_ 编码器时单位为mm, 时间编码时为s
...
int ParameterProcess::SAxisDistToIndex(float fDist_) const
{
	SCANNER& _scaner = m_pConfig->common.scanner ;
	int _index;
    if(_scaner.eScanEncoderType){//非零为编码器扫查轴
...
WDATA* ParameterProcess::GetDataAbsolutePosPointer(int nGroupId_, int nScanPos_, int nLawId_, WDATA* pSource_)
{
	...
	if(nScanPos_ < 0)
		return NULL;
	if(nScanPos_ >= GetScanMax()) // GetScanMax 最大帧数 ?
		return NULL;
...
{
	WDATA* _pShadow = GetShadowDataPointer(); //备份数据？
	WDATA*   _pData = GetDataAbsolutePosPointer(nGroupId_, nScanPos_, nLawId_, _pShadow);/*获取数据绝对位置指针？*/
...
{
    WDATA* _pData  = GetShadowDataPointer();//获取阴影数据指针？
//	if(!_pData) return false;

    int    _nPointQty = GetGroupPointQty(nGroupId_);//获取组点数量
    int _nFrameOffset = GetTotalDataSize() ;//获取全部数据大小？
    int _nGroupOffset = GetGroupDataOffset(nGroupId_); //获取组数据偏移量
    int _nLawSize;
//    if(m_pConfig->group[nGroupId_].law.eLawType == setup_LAW_TYPE_TFM){
