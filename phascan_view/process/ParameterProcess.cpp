...
int ParameterProcess::SAxisDistToIndex(float fDist_) const
{
    ...
    if(_scaner.eScanEncoderType) {
        ...
        _index = round((fDist_ - _scaner.fScanStart) / _scaner.fScanStep) ;
	} else {
        ...
        _index =  round((fDist_ * _scaner.fPrf  - _scaner.fScanStart) / _scaner.fScanStep);
	}
	return _index;
}
...
//	if(!pData_) return false;
    ...
    if(config->m_mode==D_MODE&&config->DrawCviewflage==true)cursorOffset=1;
