...
void MainWindow::slotLeftTabRightButtonDoubleClicked(int nId_)
{
	return;
}
...
//	if(_nQty <= 2)  return  ;
	...
	case setup_LANG_CHINESS:
		messageBox.setText(tr("要删除当前视图吗？"));
		_pBtnYes = (QAbstractButton*)messageBox.addButton(tr("确定"), QMessageBox::ActionRole);
		_pBtnNo  = (QAbstractButton*)messageBox.addButton(tr("取消"), QMessageBox::ActionRole);
		break;
	default:
		return;
	}
...
void MainWindow::SetToolBarName(setup_LANG eLang)
{
	...
	for(int i = 0; i < m_nTBCnt; i++)
	{
		m_actions[i]->setText(QString(tr(g_strToolBarName[i][_idx])));
	}
/*	switch(eLang)
	{
	case setup_LANG_ENGLISH:
...
void MainWindow::NewConfigure()
{
	...
	if (messageBox.clickedButton() == _pBtnYes)
	{
		...
		UpdateTableLeft() ;
		this->setWindowTitle(QString("Doppler : New Setting"));
	}
...
void MainWindow::OpenFilePro(QString strFileName_)
{
	...
	this->setWindowTitle(QString("Doppler : ") + strFileName_);
}
...
void MainWindow::SaveFile()
{
	...
	this->setWindowTitle(QString("Doppler : ") + _strFileName);
	DopplerConfigure* _pConfig = DopplerConfigure::Instance() ;
