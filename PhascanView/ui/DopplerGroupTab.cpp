...
void DopplerGroupTab::UpdateWedgeConfig(int bIsRx_)
{
	...
	else if(m_pConfig->AppEvn.eLanguage == setup_LANG_CHINESS)
	{
		...
		_str[2].sprintf("楔块类型: %s 楔块方向: %s",
						_wedge.eType ? "UT" : "PA" , _wedge.eDirection ? QString(tr("正向")).toUtf8().data() : QString(tr("反向")).toUtf8().data());
...
void DopplerGroupTab::SetWndName(setup_LANG eLang)
{
	...
	case setup_LANG_CHINESS:
		ui->toolBox->setItemText(0, QString(tr("超声设置")));
		ui->toolBox->setItemText(1, QString(tr("探头/楔块")));
		ui->toolBox->setItemText(2, QString(tr("聚焦法则")));
		ui->toolBox->setItemText(3, QString(tr("闸门和曲线")));
		ui->toolBox->setItemText(4, QString(tr("几何结构")));
		ui->toolBox->setItemText(5, QString(tr("测量")));
		ui->toolBox->setItemText(6, QString(tr("显示")));
		ui->BoxGeneral->setTitle(QString(tr("常规")));
		ui->LabelGain->setText(QString(tr("增益:")));
		ui->LabelStart->setText(QString(tr("起点:")));
		ui->LabelCurrentAngle->setText(QString(tr("当前角度:")));
		ui->LabelRange->setText(QString(tr("范围:")));
		ui->LabelVelocity->setText(QString(tr("声速:")));
		ui->LabelWedgeDelay->setText(QString(tr("楔块延迟:")));
		ui->LabelTravelMode->setText(QString(tr("声轴单位:")));//(QString(tr("跟踪模式:")));
		ui->BoxPulserReceiver->setTitle(QString(tr("发射/接收")));
		ui->LabelPulser->setText(QString(tr("脉冲:")));
		ui->LabelReceiver->setText(QString(tr("接收:")));
		ui->LabelFilter->setText(QString(tr("滤波:")));
		ui->LabelTxRxMode->setText(QString(tr("TX RX 模式:")));
		ui->LabelRectifier->setText(QString(tr("检波方式:")));//(QString(tr("整流器:")));
		ui->LabelViedoFilter->setText(QString(tr("视频滤波:")));
		ui->CheckVideoFilter->setText(QString(tr("开/关")));
		ui->LabelPointQty->setText(QString(tr("采样点:")));//(QString(tr("点数:")));
		ui->CheckPointQtyAuto->setText(QString(tr("自动")));
		ui->LabelSumGain->setText(QString(tr("总增益:")));
		ui->CheckSumGainAuto->setText(QString(tr("自动")));
		ui->LabelAverage->setText(QString(tr("平均:")));//(QString(tr("平均值:")));
		ui->LabelGroupMode->setText(QString(tr("组模式:")));
		ui->BoxProbeTrigger->setTitle(QString(tr("Tx 探头")));
		ui->BoxWedgeTrigger->setTitle(QString(tr("Tx 楔块")));
		ui->BoxProbeTrigger_2->setTitle(QString(tr("Rx 探头")));
		ui->BoxWedgeTrigger_2->setTitle(QString(tr("Rx 楔块")));
		ui->LabelLawType->setText(QString(tr("聚焦法则类型:")));
		ui->BtnProcessLaw->setText(QString(tr("进行")));
		ui->BoxAngle->setTitle(QString(tr("角度")));
		ui->LabelAngleType->setText(QString(tr("角度类型:")));
		ui->LabelAngStart->setText(QString(tr("起始角")));
		ui->LabelAngStop->setText(QString(tr("终止角")));
		ui->LabelAngStep->setText(QString(tr("步进")));
		ui->LabelPriSteering->setText(QString(tr("主轴偏转角:")));
		ui->LabelSecSteering->setText(QString(tr("次轴偏转角:")));
		ui->LlabelRefract->setText(QString(tr("折射角:")));
		ui->LlabelBeamSkew->setText(QString(tr("声束转向角:")));
		ui->BoxFocalPoint->setTitle(QString(tr("焦点")));
		ui->LabelFocalType->setText(QString(tr("聚焦类型:")));
		ui->LabelFocalStart->setText(QString(tr("起点")));
		ui->LabelFocalStop->setText(QString(tr("终点")));
		ui->LabelFocalStep->setText(QString(tr("步距")));
		ui->LabelFocalPosition->setText(QString(tr("位置:")));
		ui->LabelFocalOffset->setText(QString(tr("偏移:")));
		ui->LabelFocalDepth->setText(QString(tr("深度:")));
		ui->BoxAperture->setTitle(QString(tr("孔径")));
		ui->LlabelPriAxis->setText(QString(tr("主轴")));
		ui->LlabelSecAxis->setText(QString(tr("次轴")));
		ui->LabelElementSelect->setText(QString(tr("阵元数:")));
		ui->LabelElementStart->setText(QString(tr("起始阵元:")));
		ui->LabelElementStop->setText(QString(tr("末阵元:")));
		ui->LabelElementStep->setText(QString(tr("阵元步进:")));
		ui->LabelGate->setText(QString(tr("闸门")));
		ui->CheckGateShow->setText(QString(tr("显示")));
		ui->LabelGateSelect->setText(QString(tr("选择:")));
		ui->LabelGateStart->setText(QString(tr("起点:")));
		ui->LabelGateWidth->setText(QString(tr("宽度:")));
		ui->LabelGateHeight->setText(QString(tr("阈值")));//(QString(tr("高度:")));
		ui->LabelGateSync->setText(QString(tr("同步.:")));
		ui->LabelGateMeasure->setText(QString(tr("测量:")));

		ui->LabelSizingCurves->setText(QString(tr("曲线")));
		ui->LabelCurveSel->setText(QString(tr("类型:")));
		ui->DacPointPos->setText(QString(tr("序号:")));
		ui->BtnAddPoint->setText(QString(tr("增加")));
		ui->BtnDelPoint->setText(QString(tr("删除")));
		for(int i = 0; i < setup_DAC_POINT_QTY; i++) {
			m_pSizingCurveName[i]->setText(QString(tr("参考幅度:")));
		}
		ui->BoxPart->setTitle(QString(tr("模块")));
		ui->LabelGeometry->setText(QString(tr("结构:")));
		ui->LabelMaterial->setText(QString(tr("材料:")));
		ui->LabelPartSize1->setText(QString(tr("厚度:")));
		ui->CheckShowThickness->setText(QString(tr("显示")));
		ui->LabelPartSize2->setText(QString(tr("长度:")));
		ui->LabelPartSize3->setText(QString(tr("宽度:")));
		ui->CheckUnifiedPartSetting->setText(QString(tr("统一模块设置")));
		ui->BoxPartPosition->setTitle(QString(tr("位置")));
		ui->LabelScanOffset->setText(QString(tr("扫查偏置:")));
		ui->LabelIndexOffset->setText(QString(tr("步进偏置:")));
		ui->LabelSkewAngle->setText(QString(tr("转向角:")));

		ui->LabelPartFile->setText(QString(tr("焊缝 & 模块文件")));
		ui->LabelPartFileLoad->setText(QString(tr("加载:")));
		ui->BtnLoadPartFile->setText(QString(tr("焊缝")));
		ui->CheckPartFileShow->setText(QString(tr("显示")));
		ui->CheckLwBwShow->setText(QString(tr("显示")));
		ui->LabelTofdScanMode->setText(QString(tr("扫描模式:")));
		ui->LabelTofdProMode->setText(QString(tr("数据处理模式:")));
		ui->LabelTofdWedgeSep->setText(QString(tr("楔块间距:")));
		ui->LabelTofdPcs->setText(QString(tr("探头中心间距:")));
		ui->LabelTofdPcsCal->setText(QString(tr("计算中心间距:")));
		ui->BoxDepthCal->setTitle(QString(tr("深度校准")));
		ui->LabelDepthCal->setText(QString(tr("深度:")));
		ui->BtnDepthCal->setText(QString(tr("校准")));
		ui->LabelMeasureValue->setText(QString(tr("测量值")));
		ui->CheckMeasureShow->setText(QString(tr("显示")));
		ui->CheckBackShow->setText(QString(tr("背景")));
		ui->LabelField1->setText(QString(tr("区域 1:")));
		ui->LabelField2->setText(QString(tr("区域 2:")));
		ui->LabelField3->setText(QString(tr("区域 3:")));
		ui->LabelField4->setText(QString(tr("区域 4:")));
		ui->LabelField5->setText(QString(tr("区域 5:")));
		ui->LabelCursor->setText(QString(tr("光标")));
		ui->CheckCursorShow->setText(QString(tr("显示")));
		ui->CheckCursorSync->setText(QString(tr("光标同步")));
		ui->LabelZeroOff->setText(QString(tr("楔块延时修正值:")));
		ui->LabelDepthStart->setText(QString(tr("分层起点:")));
		ui->LabelDepthEnd->setText(QString(tr("分层终点:")));
		ui->CheckDefectShow->setText(QString(tr("显示")));
		ui->BtnDefectDelete->setText(QString(tr("删 除")));
		ui->groupBox->setTitle(QString(tr("颜色设置")));
		ui->LabelAmpColor->setText(QString(tr("波幅:")));
		ui->LabelColorRF->setText(QString(tr("射频:")));
		ui->label_5->setText(QString(tr("厚度:")));
		ui->LabelCScanSource1->setText(QString(tr("C扫查1数据源:")));
		ui->LabelCScanSource2->setText(QString(tr("C扫查2数据源:")));
		ui->LabelCScanThickness->setText(QString(tr("C扫查厚度")));
		ui->LabelCScanThicknessMin->setText(QString(tr("最小值:")));
		ui->LabelCScanThicknessMax->setText(QString(tr("最大值:")));
		ui->LabelLanguage->setText(QString(tr("语言:")));
		ui->groupBoxComDisp->setTitle(QString(tr("组合显示")));
		ui->BtnRefurbishComDisp->setText(QString(tr("刷 新")));
		break;
...
void DopplerGroupTab::UpdateSizeingCurves()
{
	...
	case setup_LANG_CHINESS:
		if(_curve.eType == setup_CURVE_TYPE_LINEAR_DAC) {
			m_pSizingCurveName[0]->setText(QString(tr("衰减系数:")));
			m_pSizingCurveName[1]->setText(QString(tr("延迟时间:")));
		} else {
			m_pSizingCurveName[0]->setText(QString(tr("参考幅度:")));
			m_pSizingCurveName[1]->setText(QString(tr("参考幅度:")));
		}
		break;
	default:
		break;
...
void DopplerGroupTab::UpdateGeometryState()
{
	...
	{
		ui->LabelPartSize1->setText(QString(tr("内部直径:")));
		ui->LabelPartSize2->setText(QString(tr("外部直径:")));
		ui->LabelPartSize3->setText(QString(tr("长度:")));
	}
	}
...
void DopplerGroupTab::on_ValuePartSize1_valueChanged(double arg)
{
	if(!ui->ValuePartSize1->hasFocus())  return ;
	PartPro();
}

void DopplerGroupTab::on_ValuePartSize2_valueChanged(double arg)
{
	if(!ui->ValuePartSize2->hasFocus())  return ;
	PartPro();
}

void DopplerGroupTab::on_ValuePartSize3_valueChanged(double arg)
{
...
void DopplerGroupTab::on_ValueCScanThicknessMin_valueChanged()
{
...
void DopplerGroupTab::on_ValueCScanThicknessMax_valueChanged()
{
