...
#include <DrawScan/DopplerDrawAScan.h>
#include <DrawScan/DopplerDrawScan.h>
#include <DrawScan/DopplerDrawBScan.h>
#include <DrawScan/DopplerDrawCScan.h>
...
int ProcessDisplay::CreateViews_AllGroups(QWidget* pWidget_)
{
	...
	for(int i= 0 ; i< _nGroupQty ; i++) {
		_pView[2*i]   = new DopplerDataView(pWidget_) ;
		_pView[2*i+1] = new DopplerDataView(pWidget_) ;
	}
	...
	for(int i = 0; i < _nGroupQty; i++) {
		...
		_split[i]->addWidget(_pView[2*i]);
		_split[i]->addWidget(_pView[2*i+1]);
	...
	int _nWidth  = pWidget_->width()  ;
	int _nHeight = pWidget_->height() ;
	int _nW   = _nWidth / _nGroupQty;
	int _nRes = _nWidth;
	QList<int> _size ;

	for(int i= 0 ; i < _nGroupQty - 1 ; i++) {
		_size.append(_nW);
		_nRes -= _nW;
	}
	_size.append(_nRes);
	...
	for(int i= 0 ; i < _nGroupQty ; i++) {
		_pView[2*i]->SetLawIdentify(0);
		_pView[2*i+1]->SetLawIdentify(0);
	}
	...
	for(int i= 0 ; i < _nGroupQty ; i++)
	{
		if(m_pConfig->group[i].eGroupMode == setup_GROUP_MODE_PA) {
			SetViewPara(_pView[2*i] , i , 0 , setup_DISPLAY_MODE_S) ;
		} else {
			SetViewPara(_pView[2*i] , i , 0 , setup_DISPLAY_MODE_B_H) ;
		}
		SetViewPara(_pView[2*i+1] , i , 0 , setup_DISPLAY_MODE_A_H) ;
	}

	QList<QWidget*>* _pList = g_pMainWnd->GetCurrentDisplayTableWidgetList();
	for(int i = 0 ; i < _nGroupQty ; i++) {
		_pList->append(_pView[2*i]);
		_pList->append(_pView[2*i+1]);
	}

	ConnectSingals(_pView , _nGroupQty * 2) ;
...
void ProcessDisplay::UpdateDataViewFrameBH(DopplerDataView* pWidget_ , int nGroupId_ , int nLawId_)
{
	...
	_process->GetBScanScanAxisRange(nGroupId_ ,  pWidget_->height() - 35 , &_fStart , &_fStop, &_fSliderStart , &_fSliderStop) ;
...
/****************************************************************************
  ...
  Input:   【nGroupId_ ： 组ID】
*****************************************************************************/
void ProcessDisplay::UpdateAllViewCursorOfGroup(int nGroupId_)
{
	int _nGroupId , _nLawId , _nDisplay  ;
	DopplerDataView* _pView  ;
	for(int j = 0 ; j < g_pMainWnd->GetDisplayTableQty() ; j++)
